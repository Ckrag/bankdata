...
import dk.ckrag.jooq.tables.Transaction.TransactionPath;
import dk.ckrag.jooq.tables.User.UserPath;
...
public class Account extends TableImpl<AccountRecord> {
     ...
     */
    public final TableField<AccountRecord, Long> USER_ID = createField(DSL.name("user_id"), SQLDataType.BIGINT, this, "");
    ...
    public List<ForeignKey<AccountRecord, ?>> getReferences() {
        return Arrays.asList(Keys.ACCOUNT__FK_USER);
    }
    ...
    public UserPath user() {
        if (_user == null)
            _user = new UserPath(this, Keys.ACCOUNT__FK_USER, null);

        return _user;
    }

    private transient TransactionPath _fkFromAccount;

    /**
     * Get the implicit to-many join path to the <code>public.transaction</code>
     * table, via the <code>fk_from_account</code> key
     */
    public TransactionPath fkFromAccount() {
        if (_fkFromAccount == null)
            _fkFromAccount = new TransactionPath(this, null, Keys.TRANSACTION__FK_FROM_ACCOUNT.getInverseKey());

        return _fkFromAccount;
    }

    private transient TransactionPath _fkToAccount;

    /**
     * Get the implicit to-many join path to the <code>public.transaction</code>
     * table, via the <code>fk_to_account</code> key
     */
    public TransactionPath fkToAccount() {
        if (_fkToAccount == null)
            _fkToAccount = new TransactionPath(this, null, Keys.TRANSACTION__FK_TO_ACCOUNT.getInverseKey());
